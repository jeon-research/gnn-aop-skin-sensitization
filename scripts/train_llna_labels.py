"""Train AblationGNN on the 436-molecule LLNA-labeled subset. Uses the
AttentiveFP backbone with llna_result as the only sensitization
supervision; no auxiliary heads.

Inputs:  data/processed/causal_aop_comprehensive_v6.csv
Outputs: results/llna_training/seed_<N>/{best_model.pt,metrics.json,history.json}

Usage:
    python3 scripts/train_llna_labels.py \\
...
from senslib.llna_loader import load_dataset_llna, load_dataset_shuffled, persist_split


from senslib.seeds import PRIMARY as DEFAULT_SEEDS  # noqa: E402
OUTPUT_ROOT_LLNA    = REVISION_ROOT / 'results' / 'llna_training'
OUTPUT_ROOT_SHUFFLE = REVISION_ROOT / 'results' / 'shuffle_control'
...
def train_one_seed(seed: int, n_epochs: int, batch_size: int, patience: int,
                   lr: float, label_col: str, shuffle_labels: bool,
                   pos_weight_override: float = None,
                   output_tag: str = None):
    set_seed(seed)
    device = setup_device()

    label_source = 'shuffled_llna' if shuffle_labels else 'llna'
    out_root = OUTPUT_ROOT_SHUFFLE if shuffle_labels else OUTPUT_ROOT_LLNA
    if output_tag:
        out_root = out_root.parent / f'{out_root.name}_{output_tag}'
    out_dir = out_root / f'seed_{seed}'
    out_dir.mkdir(parents=True, exist_ok=True)

    # Load data (optionally shuffled)
    if shuffle_labels:
        df, train_idx, val_idx, test_idx = load_dataset_shuffled(seed, label_col=label_col)
    else:
        df, train_idx, val_idx, test_idx = load_dataset_llna(seed)
    persist_split(seed, label_source, df, train_idx, val_idx, test_idx)

    print(f"\n[seed={seed}] {label_source} training")
    print(f"  dataset: {len(df)} mols, label={label_col}")
    ...
    train_g, train_y = build_split(df, train_idx, label_col, graphs, orig_to_graph)
    val_g,   val_y   = build_split(df, val_idx,   label_col, graphs, orig_to_graph)
    test_g,  test_y  = build_split(df, test_idx,  label_col, graphs, orig_to_graph)
    ...
    # pos_weight from training data for imbalance handling
    if train_y.size == 0:
    ...
    print(f"  class balance: pos={int(n_pos)} neg={int(n_neg)}  "
          f"data_pos_weight={data_pos_weight:.2f}  "
          f"using={pos_weight:.2f}")
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        if since_best >= patience:
            print(f"  early stop at epoch {epoch} (no val-AUC improvement for {patience} epochs)")
            break
